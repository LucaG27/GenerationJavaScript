...
function writeSong(event){
	...
	let input_id = originator.getAttribute('data-song-id');
	let input_title = originator.getAttribute('data-song-title');
	let input_author = originator.getAttribute('data-song-author');
	let input_composer = originator.getAttribute('data-song-composer');
	console.log("write id="+song_id);
	
	if (song_id=="") {
		//CASO ADD SONG
		//TODO SVUOTARE IL FORM
		let elementiDaSvuotare=document.getElementsByClassName("modulo_writesong");
		for (let li = 0; li < elementiDaSvuotare.length; li++) {
			elementiDaSvuotare[li].value="";
			
		}
	
	}else {
		//CASO EDIT SONG
		//RIEMPIRE IL FORM CON I DATI DELLA RIGA SPECIFICA
		document.getElementsById("input_id").value = input_id;
		document.getElementsById("input_title").value = input_title;
		document.getElementsById("input_author").value = input_author;
		document.getElementsById("input_composer").value = input_composer;
	}
...
window.addEventListener(
	...
	function(event){
		...
		let formSaveSong = document.getElementsById("formSaveSong");
